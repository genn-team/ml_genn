...
def vgg16_layer(input_layer, filters):
    x = tf.keras.layers.Conv2D(filters, 3, padding='same', use_bias=False, activation='relu',
...
def vgg16():
    ...
    x = tf.keras.layers.Dense(4096, use_bias=False, activation='relu',)(x)
    x = tf.keras.layers.Dropout(0.25)(x)
    x = tf.keras.layers.Dense(4096, use_bias=False, activation='relu',)(x)
    x = tf.keras.layers.Dropout(0.25)(x)
    x = tf.keras.layers.Dense(1000, use_bias=False, activation='softmax',)(x)
...
with tf.device('/CPU:0'):
    ...
    print(f'Accuracy of VGG16 GeNN model: {acc[0]:.16f}%')
    if args.plot:
