...
with network:
    ...
    hidden = Layer(Dense(Normal(mean=1.5, sd=0.78)), 
                   AutoNeuron([("V","scalar",0.0)], [("taum","scalar",20.0), ("theta","scalar",1.0)], {"V": "(-V+I)/taum"}, "V-theta", {"V": "0"}, solver="exponential_euler"),
    ...
    output = Layer(Dense(Normal(mean=0.93, sd=0.1)),
                   AutoNeuron([("V","scalar",0.0)], [("TauM","scalar",20.0), ("theta","scalar",1.0)], {"V": "(-V+I)/TauM"}, "V-theta", {"V": "0"}, solver="exponential_euler", readout="first_spike_time"),
                   NUM_OUTPUT, AutoSyn(vars=[("I","scalar",0.0)],params=[("taus","scalar",5.0)],ode={"I": "-I/taus"},jumps={"I": "I+w"},w_name="w",inject_current="I",solver="exponential_euler"),record_spikes=True)
