...
with network:
    ...
    # Connections
    in_to_hid = Connection(input, hidden, Dense(Normal(mean=0.03, sd=0.01)),
               Exponential(5.0))
    hid_to_hid = Connection(hidden, hidden, Dense(Normal(mean=0.0, sd=0.02)),
               Exponential(5.0))
    hid_to_out = Connection(hidden, output, Dense(Normal(mean=0.0, sd=0.03)),
               Exponential(5.0))
...
if TRAIN:
    compiler = EventPropCompiler(example_timesteps=max_example_timesteps,
                                 ...
                                 clamp_weight_conns= {in_to_hid: (-1.,1.),
                                 hid_to_hid: (-1.,1.),hid_to_out: (-1.,1.)},
    )
