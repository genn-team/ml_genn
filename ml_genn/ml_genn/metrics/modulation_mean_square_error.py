...
class ModulationMeanSquareError(Metric):
    ...
    def update(self, y_true: np.ndarray, y_pred: np.ndarray,
        ...
        if y_true[1].shape != y_pred.shape:
            raise RuntimeError(f"Prediction shape:{y_pred.shape} does "
                               f"not match label shape:{y_true[1].shape}")

        # Add mean square error between truth and prediction
        batch_sum_mse = np.mean(np.square(y_true[1] - y_pred - y_true[0]))

        # Add shape of true
        batch_total = y_true.shape[1]
