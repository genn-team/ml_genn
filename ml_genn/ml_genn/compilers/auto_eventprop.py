...
def linear_euler(vars,exprs):
    ...
    for var,expr in zip(vars,exprs):
        code.append(sympy.ccode(var+expr*dt, assign_to= "_"+str(var)))
    for var in vars:
        name = str(var)
        code.append(f"{name} = _{name}")
    return code
...
for i,j in enumerate(jumps):
    jump_c.append(sympy.ccode(j, assign_to= "_"+adj_name[i]))
for var in adj:
    jump_c += [f"{var} = _{var}"]
