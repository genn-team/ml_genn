...
from ml_genn.layers import Layer, ConvTranspose2DSynapses
...
class ConvTranspose2D(Layer):
    ...
    def __init__(self, name, filters, conv_size, conv_strides=None,
        ...
        super(ConvTranspose2D, self).__init__(name, neurons)
    ...
    def connect(self, sources):
        synapses = [ConvTranspose2DSynapses(
            self.filters, self.conv_size, self.conv_strides, self.conv_padding,
            self.connectivity_type) for i in range(len(sources))]
        super(ConvTranspose2D, self).connect(sources, synapses)
